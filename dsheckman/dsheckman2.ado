...
program dsheckman2
	version 16.0
        ...
        _vce_parserun dsheckman2 : `0'
        if "`s(exit)'" != "" {
                ereturn local cmdline `"dsheckman2 `0'"'
                exit
        }
...
program Compute, eclass
	...
	local myctr : list zvars - xvars
	qui dsregress `y1' `xvars' `lambda' if `touse_main', 	///
		controls(`myctr') `selopts'
